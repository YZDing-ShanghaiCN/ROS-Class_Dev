...
class PID_Controller:
    ...
    def get_output(self, error):
        ...
        # PID的输出
        output = self.kp * self.error + self.kd * self.error_diff + self.ki * self.error_sum
        # 限制输出
...
def velocity_publisher():
    ...
    move_er_threshold = 2e-2
    rotate_er_threshold = 1e-2
    
    # PID控制器部分实例化，PID参数可调
    # 输入合适的PID参数
    move_controller = PID_Controller(0.5, 1e-6, 0, 0, 4)  
    rotate_controller = PID_Controller(3, 0, 0, 0.01, math.pi / 2)
    ...
    while not rospy.is_shutdown():
        ...
        # mode3: 使得小乌龟旋转math.pi / 2，画出矩形的直角部分
            error = math.pi - theta
            if abs(error) < rotate_er_threshold: #如果到达目标位置（误差小于阈值）
                state = state + 1 #进入下一个状态
                cmd_vel.linear.x = 0
        ...
        # mode5: 使得小乌龟旋转math.pi / 2，画出矩形的直角部分
            error = 3 * math.pi / 2 - theta
            if abs(error) < rotate_er_threshold: #如果到达目标位置（误差小于阈值）
                state = state + 1 #进入下一个状态
                cmd_vel.linear.x = 0
                cmd_vel.angular.z = 0
        ...
        # mode7: 使得小乌龟旋转math.pi / 2，画出矩形的直角部分
