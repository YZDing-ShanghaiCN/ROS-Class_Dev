...
class KalmanFilter(object):
    ...
    def _correction_Step(self, y):
        #innovation = 
        correction_A = np.dot(self.P, self.C.T)
        correction_B = np.linalg.inv(np.dot(np.dot(self.C, self.P), self.C.T) + self.Sigma_v)
        correction_C = np.dot(correction_A, correction_B)
        self.x = self.x + np.dot(correction_C, y - np.dot(self.C, self.x))
        self.P = self.P - np.dot(np.dot(correction_C, self.C), self.P)
        return self.x, self.P
    ...
    def control_moment(self, u_new, time_now):
        self.u = u_new
        return self._predict_Step(time_now)
    ...
    def observe_moment(self, y_new, time_now):
        self.x, self.P = self._predict_Step(time_now)
        return self._correction_Step(y_new)
...
class Localization(object):
    ...
    def callback_control(self, twist):
        ...
        control_signal = np.array(twist.linear.x, twist.angular.z])
    ...
    def callback_observe(self, laserscan):
        ...
        observed_data = np.array(laserscan.ranges)

        # call observe moment function in Kalman filter
        current_time = rospy.get_time
        self.kf.control_moment(observed_data,current_time)
